...
function understrap_pagination() {
	...
	if ( ! in_array( 1, $links ) ) {
		...
		printf( '<li %s><a class="page-link" href="%s"><i class="fa fa-angle-double-left" aria-hidden="true"></i></a></li>' . "\n",
		$class, esc_url( get_pagenum_link( 1 ) ), '1' );

		/**    Previous Post Link */
		if ( get_previous_posts_link() ) {
			printf( '<li class="page-item page-item-direction page-item-prev"><span class="page-link">%1$s</span></li> ' . "\n",
			get_previous_posts_link( '<i class="fa fa-angle-left" aria-hidden="true"></i><span class="sr-only">Previous page</span>' ) );
		}
	...
	if ( get_next_posts_link() ) {
		printf( '<li class="page-item page-item-direction page-item-next"><span class="page-link">%s</span></li>' . "\n",
			get_next_posts_link( '<i class="fa fa-angle-right" aria-hidden="true"></i><span class="sr-only">Next page</span>' ) );
	}
	...
	if ( ! in_array( $max, $links ) ) {
		...
		printf( '<li %s><a class="page-link" href="%s" aria-label="Next"><span aria-hidden="true"><i class="fa fa-angle-double-right" aria-hidden="true"></i></span><span class="sr-only">%s</span></a></li>' . "\n",
